...
```{r}
secchi = read_xlsx(file.path(is_dir, 'Secchi_completedataset.xlsx')) %>% 
  mutate(date = as.Date(date)) 
str(secchi)

ls = read.csv(file.path(coll_dir, 'Yojoa_LandsatC2_SRST_filtered_v2023-03-16.csv')) %>% 
  mutate(date = as.Date(date))
str(ls)
```
...
```{r}
write.csv(sameDay_matchups, file.path(match_dir, 'sameDay_LS-Secchi_matchups.csv'), row.names = F)
write.csv(oneDay_matchups, file.path(match_dir, 'oneDay_LS-Secchi_matchups.csv'), row.names = F)
write.csv(twoDay_matchups, file.path(match_dir, 'twoDay_LS-Secchi_matchups.csv'), row.names = F)
write.csv(threeDay_matchups, file.path(match_dir, 'threeDay_LS-Secchi_matchups.csv'), row.names = F)
write.csv(fiveDay_matchups, file.path(match_dir, 'fiveDay_LS-Secchi_matchups.csv'), row.names = F)
```
