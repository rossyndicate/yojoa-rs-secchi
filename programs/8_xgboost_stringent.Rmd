---
title: "xgboost algorithm for Secchi - moderate data stringency"
...
In this 'more paranoid' approach, we use a train-test-validation method,
where the validation is holdout data from the train-test to confirm
model performance. The best performing model here was the local
knowledge matchup window with both the 7-day met summary and the
...
For the five day matchup dataset, let's grab 60% of the data as the
'train' set, and split the remainder between 'test' and 'val'.
...
optimized_booster_3d_71m <- mod_summary_3d_71m$mod[2][[1]]
...
evals_3 <- preds_3 %>%
  summarise(across(c(pred_secchi_3d_5m, pred_secchi_3d_3m, pred_secchi_3d_71m),
...
optimized_booster_5d_5m <- mod_summary_5d_5m$mod[13][[1]]
optimized_booster_5d_71m <- mod_summary_5d_71m$mod[5][[1]]
...
evals_5 <- preds_5 %>%
  summarise(across(c(pred_secchi_5d_5m, pred_secchi_5d_3m, pred_secchi_5d_71m),
                   list(rmse = ~rmse(secchi, .),
                        mae = ~mae(secchi, .),
                        mape = ~mape(secchi, .),
                        bias = ~bias(secchi, .),
...
evals_jd <- preds_jd %>%
  summarise(across(c(pred_secchi_jd_5m, pred_secchi_jd_3m, pred_secchi_jd_71m),
                   list(rmse = ~rmse(secchi, .),
                        mae = ~mae(secchi, .),
                        mape = ~mape(secchi, .),
                        bias = ~bias(secchi, .),
...
full_stack_simp <- full_stack %>%
  mutate(secchi = predict(optimized_booster_jd_71m, stack_xgb)) %>%
...
ggplot(situ_stack %>%
  ...
  labs(title = 'Yojoa Secchi Historical Record - Site E',
       subtitle = '7/1 day window, 7/1 day met, moderate data stringency',
...
```{r}
feat_importance = xgb.importance(feature_names = band_met71_feats,
               model = optimized_booster_jd_71m)
