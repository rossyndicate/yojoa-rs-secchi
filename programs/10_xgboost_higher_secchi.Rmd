---
title: "xgboost algorithm for Secchi - very stringent, high secchi focus"
...
[here](%5Bhttps://www.sciencedirect.com/science/article/pii/S0048969722015479).](<https://www.sciencedirect.com/science/article/pii/S0048969722015479>).)

In this 'extra paranoid' an 'deep Secchi' approach, we subset the
Secchi-Landsat matchups by Landsat flyover, resulting in no image-date
crossover between the train, test, or validation datasets. We then
create a weighted feature that awards the model for estimating higher
Secchi values disproportionate to lower Secchi values.
...
jemma = read.csv(file.path(match_dir, match[grepl('jemma', match) & !grepl('us', match)])) %>%
  prepData(.) %>%
  mutate(weight = secchi/max(secchi)*10)
...
optimized_booster_5d_3m <- mod_summary_5d_3m$mod[19][[1]]
...
```{r}
optimized_booster_jd_3m <- mod_summary_jd_3m$mod[4][[1]]
optimized_booster_jd_5m <- mod_summary_jd_5m$mod[5][[1]]
optimized_booster_jd_71m <- mod_summary_jd_71m$mod[5][[1]]
...
The best performing model for the validation dataset was the 7/1-day
window with the 3-day ERA5 summary. Here, we'll apply that model to the full stack, visualize the
...
```{r}
feat_importance = xgb.importance(feature_names = band_met3_feats,
               model = optimized_booster_jd_3m)
