...
match = read.csv(file.path(match_dir, 'fiveDay_LS-Secchi_matchups_n256.csv'))
...
| Landsat Mission | Number of Unique Scenes                | Number of Unique Scenes with ±5 day matchups |     |
|-------------------|--------------------|--------------------------------|--|
| Landsat 5       | `r nrow(ls5_tot)`                      | `r nrow(ls5_match)`                          |     |
| Landsat 7       | `r nrow(ls7_tot)`                      | `r nrow(ls7_match)`                          |     |
| Landsat 8       | `r nrow(ls8_tot)`                      | `r nrow(ls8_match)`                          |     |
| Landsat 9       | `r nrow(ls9_tot)`                      | `r nrow(ls9_match)`                          |     |
| Total           | `r length(unique(stack$system.index))` | `r nrow(match)`                              |     |

: Table 1

## Secchi-RS matchups

Secchi data were matched with records from the Landsat remote sensing stack at a number of time windows: same, one, two, three, and five day windows. Windows up to ±7 days have yielded reasonable results when paired with remote sensing data (Kloiber, et al. 2002).
