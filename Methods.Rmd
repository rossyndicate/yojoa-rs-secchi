...
match71 = read_csv(file.path(match_dir, 'jemma_multiDay_LS-Secchi_matchups_n238.csv'))
```
...
We experimented with two data handling methods with varying stringency. Both methods used train-test-validation datasets, where the train and test were provided for model development and the validation were hold out data to test performance independently of model development. The first method split the data as 60% of matchups as the training dataset, 20% for testing, and 20% for validation. The very stringent method split the Landsat image-dates with the same proportions, so that there was no image-date crossover between the train, test, or validation dataset.

We used a grid search method to determine optimal hyperparameters for the model, partitioning the top 20 performing models from the grid search, as measured by lowest RMSE. From these models, we selected the booster that had the lowest RMSE and a train-test RMSE that was within 0.15m to avoid selecting an overfit model. If no models met these conditions the one with the closest train-test RMSE was selected as the optimal {xgboost} model.

## Stepwise Regression Model

We used the R packages {caret} (Kuhn, 2008) and {leaps} (??TLboFcba, 2020??) to perform backwards stepwise regression (CITATION). We tested the same combinations of matchup windows and meteorological data as with the {xgboost} package, except all input data were normalized to values between 0 and 1 using a min-max scaling method.
